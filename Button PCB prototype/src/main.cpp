 ...
 * @author  C Gallant
 * @date    2025-12-21
 * @brief   MCP23017 Seven-Chip Button Test with TLC5947 LED Drivers
 *
 * @description
 * Reads button inputs from seven MCP23017 I/O expanders over I2C.
 * Controls LEDs via two TLC5947s based on button presses.
 * - MCP23017 #1-7 at addresses 0x20-0x26: reads GPA6-GPA7
 * - TLC5947 #1: LED 3 = Chip1 GPA7, LED 4 = Chip1 GPA6
 * - TLC5947 #2: LED 3 = Chip2 GPA7, LED 4 = Chip2 GPA6
 *
 * @hardware
 * - ESP32-S3-DevKitC-1
 ...
 * 2025-12-21 - Added second TLC5947 for Chip2 buttons
 ******************************************************************************/
...
void setup() {
  ...
  tlc.write();
  Serial.println("TLC5947s initialized!");
}

void loop() {
  // Chip 1 buttons → TLC #1 LEDs
  bool chip1_gpa7 = !mcp[0].digitalRead(7);
  bool chip1_gpa6 = !mcp[0].digitalRead(6);
  tlc.setPWM(3, chip1_gpa7 ? 4095 : 0);
  tlc.setPWM(4, chip1_gpa6 ? 4095 : 0);

  // Chip 2 buttons → TLC #2 LEDs (channels 24-47)
  bool chip2_gpa7 = !mcp[1].digitalRead(7);
  bool chip2_gpa6 = !mcp[1].digitalRead(6);
  tlc.setPWM(24 + 3, chip2_gpa7 ? 4095 : 0);
  tlc.setPWM(24 + 4, chip2_gpa6 ? 4095 : 0);
